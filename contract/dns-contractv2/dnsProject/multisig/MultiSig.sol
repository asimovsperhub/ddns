...
contract MultiSig is IMultiSig,DaoOwner{
    ...
    constructor(address ownerC_) DaoOwner(ownerC_){
        defaultCount = 3;
    }
    ...
    modifier sigMatched(address contractAddr_,bytes32 hash_){
        ...
        if(multiSignerStore[contractAddr_].work == false){
            require(msg.sender==IDnsOwnerPub(owner).getOwnerByContract(contractAddr_),"not owner");
            _;
        }else{
