...
library LibDnsToolKit{
    ...
    //a.b.c ==> a  a.b ==>a
    function getSecondLevelName(bytes memory name_) public pure returns(bytes memory){
        bytes memory ret;
        for (uint256 i = 0; i < name_.length; i++){
            if (name_[i] == DOT_CHAR_VAL){
                ret = new bytes(i);
    ...
    function getTopFatherNameHash(bytes memory name_) public pure returns(bytes32){
        bytes memory topFName = getTopLevelName(name_);
        return keccak256(abi.encodePacked(topFName));
    }
    //a.b.ca  6-3-1
    function getTopLevelName(bytes memory name_) public pure returns(bytes memory){
        bytes memory ret;
        for(uint256 i=name_.length-1;i>=0;i--){
            if (name_[i] == DOT_CHAR_VAL){
                ret = new bytes(name_.length - i - 1);
                break;
            }
        ...
        for(uint i=0;i<ret.length;i++){
            ret[ret.length-1-i] = name_[name_.length-i-1];
        }
