...
func BatchNewSub(start, end uint64) {
	...
	dnsnameerc, _, errown := NewDnsNameErcClient()
	if errown != nil {
		...
		if mintiter != nil {
			...
			for mintiter.Next() {
				...
				if len(rnf) > 0 {
					...
					//// todo 测试
					//pricelist, _ := cost.GetAllSecondLevelNamePrice(nil, *nameHash)
					//log.Println(pricelist)
					//// price, earn := GetLenPrice(Rootname, len(c[0]), int(ev.Year))
					//mulpy := new(big.Int)
					//price := pricelist[len(c[0])]
					//y := big.NewInt(int64(ev.Year))
					//earn := mulpy.Mul(price, y)
					//log.Println("price,earn", price, earn)
					//if earnings != nil {
					//	earnings.Earnings.Add(earnings.Earnings, earn)
					//	rootearn := earnings.RootNameMap[Rootname.Name]
					//	if rootearn != nil {
					//		rootearn.Earnings.Add(rootearn.Earnings, earn)
					...
					//			RegisterOwner: owner, RegisterTime: time.Now().Unix()})
					//	} else {
